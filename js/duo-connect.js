...
(function () {
  ...
  function _renderBadge(payload) {
    ...
    const miniVl = document.getElementById('vl-duo-conn-mini');
    const sepVl  = document.getElementById('vl-conn-sep');

    // أخفِ عند التوقّف / عدم التفعيل
    if (!cfg || !cfg.enabled || payload.state === 'idle') {
      if (mini)   { mini.style.display = 'none'; }
      if (sep)    { sep.style.display  = 'none'; }
      if (miniVl) { miniVl.style.display = 'none'; }
      if (sepVl)  { sepVl.style.display  = 'none'; }
      return;
    }

    if (!mini && !miniVl) return;
    ...
    function _applyToEl(el, sepEl) {
      if (!el) return;
      const dot = el.querySelector('.dcm-dot');
      const lbl = el.querySelector('.dcm-label');
